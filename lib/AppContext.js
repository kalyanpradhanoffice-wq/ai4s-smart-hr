...
export const calculateAttendanceStatus = (punchIn, punchOut, userLeaves, date, regularized = false) => {
    // ---- PRIORITY 0: Regularized (Force Present/Late based on punches) ----
    if (regularized && (punchIn || punchOut)) {
        const s = evaluateStatus(punchIn, punchOut);
        return s === 'absent' ? 'present' : s; // Force present if regularized
    }
    // ---- PRIORITY 1: Actual Punches (Presence always wins) ----
    if (punchIn || punchOut) {
        // We still need to call evaluateStatus to see if it's 'late', 'half-day' or 'present'
        return evaluateStatus(punchIn, punchOut);
    }
...
export const evaluateStatus = (punchIn, punchOut, shiftIn = '10:00', shiftOut = '19:00') => {
    const parseTime = (t) => {
        if (!t) return 0;
    ...
    // Grace period for late entry (15 mins)
    const graceIn = targetIn + 15;
...
export function AppProvider({ children }) {
            ...
            if (session?.user) {
                ...
                if (profile) {
                    ...
                    const normalized = {
                        ...
                        shiftIn: profile.shift_in || '10:00',
                        shiftOut: profile.shift_out || '19:00',
                        salary: {
        ...
        try {
            ...
            if (profiles) {
                setUsers(profiles.map(p => ({
                    ...
                    shiftIn: p.shift_in || '10:00',
                    shiftOut: p.shift_out || '19:00',
                    salary: {
    ...
    const value = {
        ...
        getAttendanceStatus: (userId, date) => {
            ...
            return calculateAttendanceStatus(att?.punchIn, att?.punchOut, userLeaves, date, att?.regularized);
        },
