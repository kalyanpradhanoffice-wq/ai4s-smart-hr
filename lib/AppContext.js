...
export function AppProvider({ children }) {
    ...
    const markAttendance = useCallback(async (userId, date, type, time, location) => {
        ...
        if (type === 'in') {
            ...
            if (error) {
                ...
                console.log('Punch In Successful:', data[0]);
                setAttendance(prev => {
                    const filtered = prev.filter(a => !(a.userId === userId && a.date === date));
                    return [...filtered, { ...data[0], userId: data[0].user_id, punchIn: data[0].punch_in, punchOut: data[0].punch_out }];
                });
            ...
            if (error) {
                ...
                console.log('Punch Out Successful:', data[0]);
    ...
    const rejectRegularization = useCallback(async (regId, approverId, comments) => {
        ...
        if (updatedReg) {
            ...
            logActivity({ module: 'Attendance', action: 'Regularization Rejected', performedById: approverId, targetEmployeeId: reg.employeeId, description: `Regularization for ${reg.date} rejected`, referenceId: regId });
        }
    }, [regularizations, addNotification, logActivity]);
