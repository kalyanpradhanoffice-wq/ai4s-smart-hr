...
export function AppProvider({ children }) {
    ...
    const fetchAllData = async (userId) => {
        try {
            // 1. Fetch all profiles for the directory
            ...
            setUsers(profiles.map(p => ({
                ...p,
                employeeId: p.employee_id // Map DB field to local state field
            })));

            // 2. Fetch other data (Leaves, Attendance, etc.)
            ...
            const { data: okrData } = await supabase.from('okrs').select('*');
            if (okrData) setOkrs(okrData);

            const { data: history } = await supabase.from('activity_history').select('*').order('timestamp', { ascending: false });
    ...
    const updateUser = useCallback(async (userId, updates) => {
        try {
            ...
            logActivity({ module: 'Employee', action: 'Profile Updated', performedById: currentUser?.id, targetEmployeeId: userId, description: 'Employee profile updated in cloud', referenceId: userId });
    ...
    const deactivateUser = useCallback(async (userId) => {
        try {
            ...
            addAuditEntry(currentUser?.id, 'USER_DEACTIVATED', userId, `Deactivated user profile (Retired)`);
            logActivity({ module: 'Employee', action: 'Employee Retired', performedById: currentUser?.id, targetEmployeeId: userId, description: 'Employee marked as inactive/retired', newValue: 'inactive', referenceId: userId });
    ...
    const deleteUser = useCallback(async (userId) => {
        try {
            ...
            addAuditEntry(currentUser?.id, 'USER_DELETED', userId, `Permanently deleted user: ${userId}`);
            logActivity({ module: 'Employee', action: 'Employee Deleted', performedById: currentUser?.id, targetEmployeeId: userId, description: `Employee permanently removed from cloud database`, referenceId: userId });
