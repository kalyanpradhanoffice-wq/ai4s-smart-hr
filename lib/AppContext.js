...
export const calculateAttendanceStatus = (punchIn, punchOut, userLeaves, date, regularized = false, dynamicHolidays = [], systemSettings = {}, userShift = null) => {
    // ---- PRIORITY 0: Regularized (Force Present/Late based on punches) ----
    if (regularized && (punchIn || punchOut)) {
        const s = evaluateStatus(punchIn, punchOut, systemSettings, userShift);
        return s === 'absent' ? 'present' : s; // Force present if regularized
    }
    // ---- PRIORITY 1: Actual Punches (Presence always wins) ----
    if (punchIn || punchOut) {
        return evaluateStatus(punchIn, punchOut, systemSettings, userShift);
    }
...
export const evaluateStatus = (punchIn, punchOut, systemSettings = {}, userShift = null) => {
    const parseTime = (t) => {
        if (!t) return 0;
    ...
    // Use user-specific shift if provided, otherwise fallback to system settings, or ultimate defaults
    const shiftIn = userShift?.in || systemSettings.shift_start || '10:00';
    const shiftOut = userShift?.out || systemSettings.shift_end || '19:00';
    const graceMin = Number(systemSettings.grace_period) || 0; // Default to 0 if not set

    const inMin = parseTime(punchIn);
    const outMin = parseTime(punchOut);
    const targetIn = parseTime(shiftIn);
    const targetOut = parseTime(shiftOut);

    // Grace period for late entry
    const graceIn = targetIn + graceMin;

    // Total working time
    const duration = outMin - inMin;
    const halfDayThreshold = 4.5 * 60; // 4.5 hours
    const fullDayThreshold = 8 * 60; // 8 hours

    if (duration < halfDayThreshold) return 'half-day';
    if (inMin > graceIn) return 'late';
