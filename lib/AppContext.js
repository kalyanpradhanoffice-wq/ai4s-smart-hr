...
export function AppProvider({ children }) {
    ...
    const fetchAllData = async (userId) => {
        try {
            ...
            if (attData) setAttendance(attData.map(a => ({ ...a, userId: a.user_id })));
            ...
            if (regs) setRegularizations(regs.map(r => ({ ...r, employeeId: r.employee_id })));
    ...
    const requestRegularization = useCallback(async (regData) => {
        ...
        if (data) {
            const newReg = { ...data[0], employeeId: data[0].employee_id };
    ...
    const adjustLeaveBalance = useCallback(async (userId, leaveType, delta, reason) => {
        ...
        if (currentBalance) {
            ...
            const { data } = await supabase.from('leave_balances').update({ [leaveType.toLowerCase()]: newVal }).eq('user_id', userId).select();
            if (data) setLeaveBalances(prev => prev.map(b => b.userId === userId ? { ...data[0], userId: data[0].user_id } : b));
        }
    }, [leaveBalances]);
