...
export function AppProvider({ children }) {
        ...
        try {
            ...
            if (data) {
                ...
                if (fullyApproved) {
                    ...
                    if (att) setAttendance(att.map(a => ({ ...a, userId: a.user_id })));
                }

                addNotification(lr.employee_id, 'approval', 'Leave Approved', `Your ${lr.type} request has been approved`);
                logActivity({ module: 'Leave', action: 'Leave Approved', actionCode: 'LEAVE_APPROVED', performedById: approverId, targetEmployeeId: lr.employee_id, description: `${lr.type} approved for ${lr.from_date} to ${lr.to_date}`, referenceId: leaveId });
    ...
    const approveRegularization = useCallback(async (regId, approverId, comments) => {
        try {
            ...
            const { data: updatedReg, error } = await supabase.from('regularizations').update({ status: 'approved', approver_id: approverId, comments }).eq('id', regId).select();
    ...
    const approveLoan = useCallback(async (loanId, approverId, comments, level, totalLevels) => {
        try {
            ...
            if (data && data[0]) {
                ...
                logActivity({ module: 'Payroll', action: 'Loan Approved', actionCode: 'LOAN_APPROVED', performedById: approverId, targetEmployeeId: loan.employee_id, targetEmployeeName: emp?.name, description: `Loan for ₹${loan.amount.toLocaleString()} approved (Level ${level})`, previousValue: 'Pending', newValue: fullyApproved ? 'Approved' : 'Pending', referenceId: loanId });
    ...
    const rejectLoan = useCallback(async (loanId, approverId, comments) => {
        try {
            ...
            if (data && data[0]) {
                ...
                logActivity({ module: 'Payroll', action: 'Loan Rejected', actionCode: 'LOAN_REJECTED', performedById: approverId, targetEmployeeId: loan.employee_id, targetEmployeeName: emp?.name, description: `Loan request for ₹${loan.amount.toLocaleString()} rejected`, previousValue: 'Pending', newValue: 'Rejected', referenceId: loanId });
