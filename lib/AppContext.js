...
export const calculateAttendanceStatus = (punchIn, punchOut, userLeaves, date, regularized = false, dynamicHolidays = [], systemSettings = {}, userShiftType = null) => {
    ...
    if (userLeaves && userLeaves.length > 0) {
        ...
        if (todayLeave) {
            return (todayLeave.type || '').toUpperCase() === 'WFH' ? 'wfh' : 'leave';
        }
    }
...
export function AppProvider({ children }) {
    ...
    useEffect(() => {
        const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
            if (session?.user) {
                ...
                if (profile) {
                    // --- ENFORCE NETWORK RESTRICTION ON SESSION RESTORE ---
                    const { data: secData } = await supabase.from('security_config').select('*').eq('id', 'system_config').single();
                    const effectiveConfig = secData?.config || DEFAULT_SECURITY_CONFIG;

                    if (effectiveConfig.wifiRestrictionEnabled) {
                        try {
                            const res = await fetch('https://api.ipify.org?format=json');
                            const ipData = await res.json();
                            const currentIp = ipData?.ip;
                            if (isNetworkRestricted(profile, effectiveConfig, { ip: currentIp })) {
                                console.warn(`[SECURITY] Auto-login blocked for ${profile.email} from unauthorized IP: ${currentIp}`);
                                await supabase.auth.signOut();
                                setCurrentUser(null);
                                setIsInitialized(true);
                                return; // Stop processing and remain logged out
                            }
                        } catch (e) {
                            console.warn('[SECURITY] Failed to verify IP on session restore:', e);
                            // If we fail to fetch IP but restrictions are on, we block
                            if (isNetworkRestricted(profile, effectiveConfig, { ip: null })) {
                                await supabase.auth.signOut();
                                setCurrentUser(null);
                                setIsInitialized(true);
                                return;
    ...
    const fetchAllData = async (userId) => {
        try {
            ...
            if (secConfig) {
                setSecurityConfig(secConfig.config || DEFAULT_SECURITY_CONFIG);
            } else {
    ...
    const login = useCallback(async (email, password, ipToVerify = null) => {
        ...
        // --- NEW: STRICT NETWORK ENFORCEMENT ---
        // Fetch absolute latest config directly from DB to prevent stale state bypass
        const { data: secData } = await supabase.from('security_config').select('*').eq('id', 'system_config').single();
        const effectiveConfig = secData?.config || DEFAULT_SECURITY_CONFIG;

        if (effectiveConfig.wifiRestrictionEnabled) {
            let finalIp = ipToVerify;
            
            // Critical Fallback: if IP wasn't detected by the browser yet, try one last time
            if (!finalIp) {
                try {
                    const res = await fetch('https://api.ipify.org?format=json');
                    ...
                    console.warn('[SECURITY] Last-minute IP fetch failed, defaulting to restricted.');
                }
            }

            if (isNetworkRestricted(profile, effectiveConfig, { ip: finalIp })) {
                console.warn(`[SECURITY] Login blocked for ${profile.email} from unauthorized IP: ${finalIp || 'UNKNOWN'}`);
                await supabase.auth.signOut(); // Kill server session immediately
