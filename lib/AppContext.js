...
import { getDashboardRoute, isLocationRestricted } from './rbac';
import { supabase } from './supabase';
import { SHIFTS, DEFAULT_SHIFT } from './shifts';

const isHolidayStatic = (date) => false; // Static holidays removed, use database instead.
...
export const calculateAttendanceStatus = (punchIn, punchOut, userLeaves, date, regularized = false, dynamicHolidays = [], systemSettings = {}, userShiftType = null) => {
    // ---- PRIORITY 0: Regularized (Force Present/Late based on punches) ----
    if (regularized && (punchIn || punchOut)) {
        const s = evaluateStatus(punchIn, punchOut, systemSettings, userShiftType);
        return s === 'absent' ? 'present' : s; // Force present if regularized
    }
    // ---- PRIORITY 1: Actual Punches (Presence always wins) ----
    if (punchIn || punchOut) {
        return evaluateStatus(punchIn, punchOut, systemSettings, userShiftType);
    }


    // ---- PRIORITY 2: Holidays ----
    const isDynamicHoliday = dynamicHolidays.some(h => (h.date || h) === date);
    if (isDynamicHoliday) return 'holiday';

    // ---- PRIORITY 3: Approved Leaves/WFH ----
    if (userLeaves && userLeaves.length > 0) {
        const todayLeave = userLeaves.find(l => {
            ...
            return date >= s && date <= e && isApproved;
        });
        if (todayLeave) {
            const lType = (todayLeave.type || '').toUpperCase();
            if (lType === 'WFH') return 'wfh';
            if (lType === 'OD') return 'od';
            return 'leave';
        }
    }

    // ---- PRIORITY 4: Weekly Off (Weekend) ----
    if (date) {
        ...
        let isWO = false;
        if (weekendType === 'sun' && day === 0) isWO = true;
        else if (weekendType === 'sat_sun' && (day === 0 || day === 6)) isWO = true;
        else if (weekendType === 'fri_sat' && (day === 5 || day === 6)) isWO = true;
...
export const isWorkingDay = (date, systemSettings = {}, dynamicHolidays = []) => {
    ...
    const isHoliday = dynamicHolidays.some(h => (h.date || h) === date);
...
export const countWorkingDays = (startDate, endDate, systemSettings = {}, dynamicHolidays = []) => {
    ...
    while (curr <= end) {
        ...
        if (isWorkingDay(dateStr, systemSettings, dynamicHolidays)) {
            count++;
        }
...
export const evaluateStatus = (punchIn, punchOut, systemSettings = {}, userShiftType = null) => {
    const parseTime = (t) => {
        if (!t) return 0;
    ...
    if ((punchIn && !punchOut) || (!punchIn && punchOut)) return 'missing_punch';

    // 2. Load Shift Config
    const shiftKey = userShiftType || systemSettings.default_shift || DEFAULT_SHIFT;
    const shift = SHIFTS[shiftKey] || SHIFTS[DEFAULT_SHIFT];
    const shiftStartMin = parseTime(shift.start);
    const inMin = parseTime(punchIn);
    const outMin = parseTime(punchOut);

    // Calculate relative "minutes since shift start" for in-punch
    // This handles the 00:00 crossover for overnight shifts (NS)
    let relativeIn = inMin - shiftStartMin;
    if (relativeIn < -720) relativeIn += 1440; // Handles punching in very early (e.g. 21:00 for 22:00 shift)
    if (relativeIn > 720) relativeIn -= 1440; // Handles crossover cases

    // 3. Working Hours Validation (The Hard Rule)
    let totalMinutes = outMin - inMin;
    if (totalMinutes < 0) totalMinutes += 1440; // 00:00 crossover

    if (totalMinutes < 180) return 'absent'; // < 3 Hours
    if (totalMinutes < 360) return 'half-day'; // 3 - 6 Hours

    // 4. Arrival Logic (Total Hours > 6)
    // Convert windows to relative minutes from start
    const getRel = (timeStr) => {
        let rel = parseTime(timeStr) - shiftStartMin;
        if (rel < -720) rel += 1440;
        if (rel > 720) rel -= 1440;
        return rel;
    };

    const presentLimit = getRel(shift.windows.present);
    const lateLimit = getRel(shift.windows.late);
    const halfDayLimit = getRel(shift.windows.halfDay);

    if (relativeIn <= presentLimit) {
        // Check Early Out
        const relOut = outMin - shiftStartMin;
        const totalShiftMinutes = (parseTime(shift.end) - shiftStartMin + 1440) % 1440;
        const earlyOutLimit = getRel(shift.windows.earlyOut);
        
        // If they stayed at least 95% of the shift, or past the early out limit
        let relativeOut = (outMin - shiftStartMin + 1440) % 1440;
        let targetOut = (parseTime(shift.start) - shiftStartMin + totalShiftMinutes + 1440) % 1440;
        
        // Actually simpler: if outMin is before earlyOut window
        const outRel = (outMin - shiftStartMin + 1440) % 1440;
        const eoRel = (parseTime(shift.windows.earlyOut) - shiftStartMin + 1440) % 1440;
        
        if (outRel < eoRel) return 'early-exit';
        return 'present';
    }
    
    if (relativeIn <= lateLimit) return 'late';
    if (relativeIn <= halfDayLimit) return 'half-day';
...
export function AppProvider({ children }) {
    ...
    const [isSecurityConfigLoaded, setIsSecurityConfigLoaded] = useState(false);
            ...
            if (session?.user) {
                ...
                if (profile) {
                    ...
                    const normalized = {
                        ...
                        shiftType: profile.shift_type || DEFAULT_SHIFT,
                        shiftIn: profile.shift_in || systemSettings.shift_start || '10:00',
                        shiftOut: profile.shift_out || systemSettings.shift_end || '19:00',
    ...
    const fetchAllData = async (userId) => {
        try {
            ...
            // Parallel fetch for better performance - prioritized and limited
            const [
                { data: profiles },
                { data: leaves },
                { data: attData },
                ...
                // Only fetch recent leaves (last 90 days) for general history
                supabase.from('leaves').select('*').order('applied_on', { ascending: false }).limit(200),
                // Only fetch recent attendance (last 60 days) to speed up initial load
                supabase.from('attendance').select('*').gte('date', new Date(Date.now() - 60 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]).order('date', { ascending: false }),
                supabase.from('okrs').select('*'),
                supabase.from('activity_history').select('*').order('timestamp', { ascending: false }).limit(100), // Reduced limit for faster response
                supabase.from('payroll').select('*').order('month', { ascending: false }).limit(100),
                supabase.from('leave_balances').select('*'),
                supabase.from('regularizations').select('*').order('requested_on', { ascending: false }).limit(100),
                supabase.from('loans').select('*').order('requested_on', { ascending: false }),
            ...
            let currentSettings = {};
            if (settingsData) {
                settingsData.forEach(s => currentSettings[s.setting_key] = s.setting_value);
                setSystemSettings(currentSettings);
            }
            if (leaveTypesData) setLeaveTypes(leaveTypesData);
            if (holidaysData) setCompanyHolidays(holidaysData);
            ...
            if (profiles) {
                setUsers(profiles.map(p => ({
                    ...
                    shiftType: p.shift_type || DEFAULT_SHIFT,
                    shiftIn: p.shift_in || currentSettings.shift_start || '10:00',
                    shiftOut: p.shift_out || currentSettings.shift_end || '19:00',
            ...
            if (okrData) setOkrs(okrData.map(o => ({
                ...o,
                userId: o.user_id,
            ...
            if (regs) {
                setRegularizations(regs.map(r => ({
                    ...
                    punchIn: formatTimeForUI(r.punch_in),
                    punchOut: formatTimeForUI(r.punch_out)
                })));
            }
            ...
            if (secConfig) {
                // Merge database config with DEFAULT_SECURITY_CONFIG to ensure new geofencing keys are present
                setSecurityConfig({ ...DEFAULT_SECURITY_CONFIG, ...secConfig.config });
            } else {
    ...
    // ---- AUTH ----
    const login = useCallback(async (email, password, ipToVerify = null) => {
    ...
    const logout = useCallback(async () => {
        try {
            if (currentUser) {
                // Fire and forget logs so they don't block logout if DB is slow
                addAuditEntry(currentUser.id, 'LOGOUT', 'system', 'User logged out (Force)').catch(console.error);
                logActivity({ module: 'Auth', action: 'User Logout', actionCode: 'LOGOUT', performedById: currentUser.id, performedByName: currentUser.name, targetEmployeeId: currentUser.id, targetEmployeeName: currentUser.name, description: `${currentUser.name} forcefully logged out`, previousValue: 'active', newValue: 'inactive' }).catch(console.error);
            }
        } catch (e) {
        ...
        if (typeof window !== 'undefined') {
            ...
            // Final redirect to login with a cache-buster
            window.location.href = `/login?logged_out=${Date.now()}`;
        }
    ...
    const updateUser = useCallback(async (userId, updates) => {
        try {
            ...
            if (updates.shiftIn) dbUpdates.shift_in = updates.shiftIn;
            if (updates.shiftOut) dbUpdates.shift_out = updates.shiftOut;
            ...
            delete dbUpdates.shiftIn;
            delete dbUpdates.shiftOut;
    ...
    // ---- SECURITY CONFIG ----
    const updateSecurityConfig = useCallback(async (updatesOrFn) => {
        try {
            setSecurityConfig(prev => {
                const combined = typeof updatesOrFn === 'function'
                    ? updatesOrFn(prev)
                    : { ...prev, ...updatesOrFn };

                console.log('--- SECURITY CONFIG UPDATE START ---');
                console.log('Payload being sent to DB:', combined);

                // Schedule the database update
                (async () => {
                    const { error } = await supabase.from('security_config').upsert({ id: 'system_config', config: combined });
                    if (error) {
                        console.error('SECURITY DB ERROR:', error);
                        addToast(`Security Save Failed: ${error.message}`, 'error');
                    } else {
                        console.log('SECURITY DB SUCCESS: Configuration saved.');
            ...
            console.error('Failed to update security config state:', err);
            addToast('Critical error updating security state', 'error');
        }
    ...
    const adjustLeaveBalance = useCallback(async (userId, leaveType, delta, reason) => {
        ...
        if (current) {
            ...
            // Start from 0 if no balance record exists
            newVal = Math.max(0, delta);
            const { error } = await supabase.from('leave_balances')
    ...
    const applyLeave = useCallback(async (leaveData) => {
        ...
        try {
            ...
            if (data && data[0]) {
                ...
                const targetApprover = leaveData.approverId || currentUser.managerId || currentUser.reportingTo;
                if (targetApprover) {
                    addNotification(targetApprover, 'leave', 'New Leave Request', `${currentUser?.name} applied for ${leaveData.type} leave`);
                }
        ...
        try {
            ...
            if (data) {
                ...
                if (fullyApproved) {
                    ...
                    const lType = lr.type.toUpperCase();

                    if (lType === 'COMPOFF_CREDIT') {
                        // Increase comp-off balance
                        await adjustLeaveBalance(lr.employee_id, 'compoff', lr.days, `Approved Comp-Off Credit: ${lr.from_date}`);
                    } else {
                        const statusType = lType === 'WFH' ? 'wfh' : (lType === 'OD' ? 'od' : 'leave');
                        ...
                        for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
                            ...
                            attendancePromises.push(
                                supabase.from('attendance').upsert({
                                    ...
                                    date: dateStr,
                                    status: statusType,
                                    location: lType === 'WFH' ? 'remote' : (lType === 'OD' ? 'on-site' : 'office')
                    ...
                    const paidLeaveTypes = ['CL', 'EL', 'SL', 'ML', 'PL', 'COMPOFF'];
    ...
    const cancelLeave = useCallback(async (leaveId) => {
        try {
            ...
            // Only allow cancellation for pending or future leaves
            const isPending = lr.status === 'pending';
            const isFuture = new Date(lr.from_date || lr.from) > new Date();

            if (!isPending && !isFuture) {
                addToast('Cannot cancel past or already approved leaves unless they are in the future', 'warning');
                return;
            }
            ...
            // If it was already approved (future leave), we might need to restore balance
            if (lr.status === 'approved') {
                const paidLeaveTypes = ['CL', 'EL', 'SL', 'ML', 'PL'];
                if (paidLeaveTypes.includes(lr.type.toUpperCase())) {
                    await adjustLeaveBalance(lr.employee_id, lr.type, lr.days, `Cancelled ${lr.type} leave: ${lr.from_date || lr.from}`);
                }
                // Also clean up attendance entries for those future dates
                const start = new Date(lr.from_date || lr.from);
                const end = new Date(lr.to_date || lr.to);
                for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
                    const dateStr = d.toISOString().split('T')[0];
                    await supabase.from('attendance').delete().eq('user_id', lr.employee_id).eq('date', dateStr).neq('status', 'present');
                }
            }

            setLeaveRequests(prev => prev.map(item => item.id === leaveId ? { ...item, status: 'cancelled' } : item));
            addToast('Leave request cancelled', 'success');
            logActivity({ module: 'Leave', action: 'Leave Cancelled', performedById: currentUser?.id, targetEmployeeId: lr.employee_id, description: `Cancelled ${lr.type} leave for ${lr.from_date || lr.from}` });

            await fetchAllData();
        } catch (error) {
            console.error('Cancel Leave Error:', error);
            addToast('Failed to cancel leave', 'error');
        }
    }, [leaveRequests, currentUser, adjustLeaveBalance, fetchAllData, logActivity, addToast]);
    // ---- ATTENDANCE ----
    const markAttendance = useCallback(async (userId, date, type, time, location, photoCaptured = false, locationVerified = false) => {
        const executeUpsert = async (useModern = true, withLocation = true, useIsoTime = false, withSecurityFlags = true) => {
            ...
            if (withSecurityFlags) {
                payload.location_verified = locationVerified;
            }
            ...
            if (error) {
                if (error.message.includes('location_verified') && withSecurityFlags) {
                    return executeUpsert(useModern, withLocation, useIsoTime, false);
                }
                if (error.message.includes('timestamp') && !useIsoTime) {
                    return executeUpsert(useModern, withLocation, true, withSecurityFlags);
                }
                if (withLocation && error.message.includes('location')) {
                    return executeUpsert(useModern, false, useIsoTime, withSecurityFlags);
                }
                if (useModern && (error.message.includes('punch_in') || error.message.includes('punch_out'))) {
                    return executeUpsert(false, withLocation, useIsoTime, withSecurityFlags);
                }
            }
            return { data, error };
        };
        ...
        const existing = attendance.find(a => a.userId === userId && a.date === date);
        if (type === 'in' && existing) return; 
        if (type === 'out' && (!existing || existing.punchOut)) return; 

        if (type === 'in' || type === 'out') {
            const { data, error } = await executeUpsert(true, true, false, true);
            if (error) {
                ...
                logActivity({
                    ...
                    targetEmployeeId: userId,
                    targetEmployeeName: emp?.name,
                    description: `${type === 'in' ? 'Punched in' : 'Punched out'} at ${timeStr} from ${location || 'office'} (Photo: ${photoCaptured ? 'Yes' : 'No'}, Location Verified: ${locationVerified ? 'Yes' : 'No'})`
                });
                addToast(`${type === 'in' ? 'Punched in' : 'Punched out'} successfully at ${timeStr}`, 'success', '🕒');
            }
        }
    }, [attendance, logActivity, users, addToast]);

    // ---- ATTENDANCE REGULARIZATION ----
    const requestRegularization = useCallback(async (regData) => {
        try {
            // Check for existing pending request for the same date
            const existingPending = regularizations.find(r => r.employeeId === regData.employeeId && r.date === regData.date && r.status === 'pending');
            if (existingPending) {
                addToast(`A request for ${regData.date} is already pending.`, 'warning', '⚠️');
                return null;
            }
            ...
            const { data, error } = await supabase.from('regularizations').insert([{
                ...
                punch_in: regData.punchIn || null,
                punch_out: regData.punchOut || null,
        ...
        try {
            ...
            if (updatedReg) {
                ...
                if (fullyApproved) {
                    ...
                    const calculatedStatus = evaluateStatus(reg.punch_in, reg.punch_out, emp?.shift_in || '10:00', emp?.shift_out || '19:00');

                    const attendancePayload = {
                        user_id: reg.employeeId,
                        date: reg.date,
                        status: calculatedStatus,
                        regularized: true,
                        punch_in: reg.punch_in,
                        punch_out: reg.punch_out,
                        clock_in: null,
                        clock_out: null
                    };

                    // Manual upsert for better compatibility with different schema versions
                    const { data: existingAtt } = await supabase.from('attendance').select('id').eq('user_id', reg.employeeId).eq('date', reg.date).maybeSingle();

                    let attSaveError = null;
                    if (existingAtt) {
                        const { error } = await supabase.from('attendance').update(attendancePayload).eq('id', existingAtt.id);
                        attSaveError = error;
                    } else {
                    ...
                    if (attSaveError) {
                        console.error('Error updating attendance after regularization:', attSaveError);
                        throw attSaveError;
                    }
    ...
    }, [regularizations, users, attendance, addNotification, logActivity, addToast]);
    ...
    const approveSalaryUpgrade = useCallback(async (suId, approverId, comments, level, totalLevels) => {
        ...
        if (data) {
            ...
            addToast(`Salary upgrade approved`, 'success', '✅');
        }
    }, [salaryUpgrades, users, fetchAllData, addAuditEntry, logActivity, addToast]);

    const rejectSalaryUpgrade = useCallback(async (suId, approverId, comments) => {
        const su = salaryUpgrades.find(s => s.id === suId);
        if (!su) return;

        const approval = { level: su.current_level, approvedBy: approverId, approvedOn: new Date().toISOString(), status: 'rejected', comments };
        const newApprovals = [...(su.approvals || []), approval];
        ...
        if (data) {
            ...
            addAuditEntry(approverId, 'SALARY_UPGRADE_REJECTED', su.id, `Rejected salary upgrade for ${su.employeeId}`);
            logActivity({
                module: 'Payroll',
                action: 'Salary Upgrade Rejected',
                actionCode: 'SALARY_UPGRADE_REJECTED',
                performedById: approverId,
                performedByName: users.find(u => u.id === approverId)?.name,
                targetEmployeeId: su.employee_id,
                targetEmployeeName: users.find(u => u.id === su.employee_id)?.name,
                description: `Salary upgrade to ₹${su.proposedSalary.toLocaleString()} rejected`,
                previousValue: 'Pending',
                newValue: 'Rejected',
                referenceId: suId
            });
            addToast(`Salary upgrade rejected`, 'warning', '✖️');
        }
    }, [salaryUpgrades, users, addAuditEntry, logActivity, addToast]);
    ...
    const value = {
        ...
        getAttendanceStatus: (userId, date) => {
            const att = attendance.find(a => (a.userId === userId || a.user_id === userId) && a.date === date);
            const userLeaves = leaveRequests.filter(l => (l.employee_id === userId || l.employeeId === userId));
            const emp = users.find(u => u.id === userId);
            // Pass user-specific shift if it exists, otherwise it defaults to system settings in calculateAttendanceStatus
            const userShift = emp?.shift_in && emp?.shift_out ? { in: emp.shift_in, out: emp.shift_out } : null;
            return calculateAttendanceStatus(att?.punchIn, att?.punchOut, userLeaves, date, att?.regularized, companyHolidays, systemSettings, userShift);
        },
        isWorkingDay: (date) => isWorkingDay(date, systemSettings, companyHolidays),
        countWorkingDays: (start, end) => countWorkingDays(start, end, systemSettings, companyHolidays),
        ...
        // Salary
        approveSalaryUpgrade, rejectSalaryUpgrade, setSalaryUpgrades,
