 ...
 * If currentAccessPoint ( {ssid, ip} ) is provided, it validates against the paired list.
 */
export function isNetworkRestricted(user, securityConfig, currentAccessPoint = null) {
    ...
    if (exemptRoles.includes(user.role)) return false;

    // 2. Network Check (if info provided)
    if (currentAccessPoint) {
        const { ssid, ip } = currentAccessPoint;
        const aps = securityConfig.allowedAccessPoints || [];

        // Match if any approved Access Point matches
        const isApproved = aps.some(ap => {
            const ssidMatch = !ssid || ap.ssid.toLowerCase() === ssid.toLowerCase();
            const ipMatch = ap.ip === ip || (ap.ip.endsWith('.0/24') && ip.startsWith(ap.ip.slice(0, -4)));

            // If we have an SSID, both must match. If we don't have SSID (browser limit), IP is enough.
            if (ssid) return ssidMatch && ipMatch;
            return ipMatch;
        });

        if (isApproved) return false; // Not restricted
    }

    // Default to restricted if policy is on and no specific bypass found
    return true;
}
